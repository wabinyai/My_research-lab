...
# Define Celery task (renamed to process_satellite_data)
@app.task
def process_satellite_data():
    ...
    start_time = datetime.now(tz=pytz.UTC) - timedelta(weeks=2)  # Adjust to run every 2 weeks
...
app.conf.beat_schedule = {
    'process-every-two-weeks': {
        'task': 'tasks.process_satellite_data',
        'schedule': timedelta(weeks=2),  # Run every 2 weeks
    },
}
